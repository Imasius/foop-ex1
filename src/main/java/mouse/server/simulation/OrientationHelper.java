...
public class OrientationHelper {
     ...
     * @return moved Point
     */
    public Point applyMotion(Point fromPosition, Orientation direction) {
        switch (direction) {
            case NORTH:
                return new Point(fromPosition.x, fromPosition.y - 1);
            case SOUTH:
                return new Point(fromPosition.x, fromPosition.y + 1);
            case EAST:
                return new Point(fromPosition.x + 1, fromPosition.y);
            case WEST:
                return new Point(fromPosition.x - 1, fromPosition.y);
        }
    ...
    /**
     * Gives a List that contains all cardinal directions, that lead from start
     * to dest
     *
     * @param start
     * @param dest
