...
public class ClientLevel extends LevelStructure implements ServerToClientMessageListener {
    ...
    public void handleUpdateMice(ArrayList<Mouse> mice) {
        log.debug("Received update for " + mice.size() + " mice");
        for (Mouse m : mice) {
            log.debug(">move:" + m.getPosition().x + "," + m.getPosition().y);
        }
        ...
        if (this.mice == null) {
            ...
            for (Mouse m : mice) {
                Mouse old = this.getMice().get(m.getPlayerIndex());
                int x = old.getPosition().x;
                int y = old.getPosition().y;
                this.mice.set(m.getPlayerIndex(), m);
                log.debug("Update mouse " + m.getPlayerIndex() + ":" + m.getPlayerIndex() + " from " + x + "," + y + "->" + m.getPosition().x + "," + m.getPosition().y);
            }
        }
    }
    
    public void handleUpdateDoors(ArrayList<Door> doors) {
        log.debug("Received update for " + doors.size() + " doors");
        for (Door door : doors) {
